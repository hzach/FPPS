...
trait Solver extends GameDef {
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val possiblePaths =
        for {
          init <- initial
          (lead, moves) = init
          (next, nextMove) <- newNeighborsOnly(neighborsWithHistory(lead, moves), explored)
        } yield (next, nextMove)

      val newlyExplored = explored ++ (possiblePaths map (_._1))
      possiblePaths ++ from(possiblePaths, newlyExplored)
    }
  }
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] =
    //need to include the startBlock in the set of explored blocks
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] =
    pathsFromStart filter ( path => path._1.b1 == goal && path._1.b2==goal)
